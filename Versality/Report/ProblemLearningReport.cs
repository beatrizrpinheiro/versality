...
{
    public class ProblemLearningReport
    {
        #region Declaration
        int _totalColumn = 3;
        Document _document;
        ...
        List<ProblemLearning> _problemLearnings = new List<ProblemLearning>();
        #endregion

        public byte[] PrepareReport(List<ProblemLearning> problemLearnings)
        {
            _problemLearnings = problemLearnings;
        ...
        private void ReportBody()
        {
            ...
            foreach (ProblemLearning problemLearning in _problemLearnings)
            {
                ...
                _pdfPCell = new PdfPCell(new Phrase(problemLearning.Name, _fontStyle));
                _pdfPCell.HorizontalAlignment = Element.ALIGN_CENTER;
                _pdfPCell.VerticalAlignment = Element.ALIGN_MIDDLE;
                _pdfPCell.BackgroundColor = BaseColor.WHITE;
                _pdfTable.AddCell(_pdfPCell);

                _pdfPCell = new PdfPCell(new Phrase(problemLearning.Solution, _fontStyle));
                _pdfPCell.HorizontalAlignment = Element.ALIGN_CENTER;
                _pdfPCell.VerticalAlignment = Element.ALIGN_MIDDLE;
                _pdfPCell.BackgroundColor = BaseColor.WHITE;
                _pdfTable.AddCell(_pdfPCell);
