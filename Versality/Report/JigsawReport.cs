...
namespace Versality.Report
{
    public class JigsawReport
    {
        ...
        List<Jigsaw> _jigsaws = new List<Jigsaw>();
        #endregion

        public byte[] PrepareReport(List<Jigsaw> jigsaws)
        {
            _jigsaws = jigsaws;
        ...
        private void ReportBody()
        {
            ...
            int serialNumber = 1;
            foreach (Jigsaw jigsaw in _jigsaws)
            {
                ...
                _pdfPCell = new PdfPCell(new Phrase(jigsaw.Name, _fontStyle));
                _pdfPCell.HorizontalAlignment = Element.ALIGN_CENTER;
                _pdfPCell.VerticalAlignment = Element.ALIGN_MIDDLE;
                _pdfPCell.BackgroundColor = BaseColor.WHITE;
                _pdfTable.AddCell(_pdfPCell);

                _pdfPCell = new PdfPCell(new Phrase(jigsaw.Solution, _fontStyle));
                _pdfPCell.HorizontalAlignment = Element.ALIGN_CENTER;
                _pdfPCell.VerticalAlignment = Element.ALIGN_MIDDLE;
                _pdfPCell.BackgroundColor = BaseColor.WHITE;
                _pdfTable.AddCell(_pdfPCell);
